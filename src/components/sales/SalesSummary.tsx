...
import { SalesFormData, CostSummary } from '@/types/transaction';
...
const SalesSummary = ({
  ...
  const handlePrint = () => {
    printDotMatrixInvoice(formData, 'sales');
  };

  const handleCopyInvoice = async () => {
    const content = getDotMatrixContent(formData, 'sales');
    try {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="form-section">
            <h3 className="text-lg font-semibold mb-4">Ringkasan Pelanggan</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">{formData.customer.phone || '-'}</p>
              </div>
              <div className="sm:col-span-2">
                <span className="text-muted-foreground">Alamat:</span>
                <p className="font-medium">{formData.customer.address}</p>
              </div>
              {formData.customer.npwp && (
                <div>
                  <span className="text-muted-foreground">NPWP/NIK:</span>
                  <p className="font-medium font-mono">{formData.customer.npwp}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="form-section">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">
                  {formData.applyVat ? 'Ya (11%)' : formData.vatExempt ? 'Bebas PPN' : 'Tidak'}
                </p>
              </div>
            </div>
          </div>
